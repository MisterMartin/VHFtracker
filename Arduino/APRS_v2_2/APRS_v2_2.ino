...
#define FLIGHT_NAME "MCM001"      //Flight name in APRS message
#define FLIGHT_PERIOD 120         // How many minutes to be in active flight mode before sleeping
#define HIBERNATE_PERIOD 30       // How many days to sleep before begining transmission
#define FLIGHT_APRS_TX_PERIOD 2   // How often to send an APRS position in minutes in flight mode
#define TRACK_APRS_TX_PERIOD 10   // How often to send APRS position in minutes in track mode
#define TRACK_GPS_PERIOD 12       // How often to look for a GPS position in hours in track mode
#define TRACK_PING_TX_PERIOD 3    // how often to send a 'beep' in seconds
#define PING_DURATION 30          // How long the ping transmission is in ms
...
/* Pin Configuration */
#define PTT_PIN 2               // Push to talk pin, pull this high to turn on the transmitter
...
#define TX_ENABLE 2             // Same as push to talk
...
long pings_sent = 5;
int flight_APRS_packets = 0;
int track_APRS_packets = 0;
int days_in_hibernate = 0;
...
void loop()
{
 ...
 if ( mode == 0) 
 {
    getGPS(90);  //try for 1 minute to get a GPS Position
 ...
 if (mode == 1) 
 {
    ...
    if (days_in_hibernate > HIBERNATE_PERIOD)
      mode = 2;
    alarm.setRtcTimer(24, 0, 0);// hour, min, sec, go back to sleep for a day
    Snooze.hibernate( config_teensy36 );    
 }
...
//    baro.begin();
//    float hPa = baro.getPressure()/100.0;
//    float altm = baro.getAltitude();
//    float tempC = baro.getTemperature();
//    float volts = analogRead(A6)/4095.0 * 3.3 * 2.0; 
//
//    char Message[24] = "";
//    sprintf(Message, "%4.1fC,%4.1fhPa,%3.2fV", tempC, hPa, volts);
 ...
 */
  ...
  // If above 5000 feet switch to a single hop path - only relevant in CONUS
  int nAddresses;
