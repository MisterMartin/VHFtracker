...
#define PRE_MSG   "1030_PRE"   // Flight name in APRS preflight mode message
#define TRK_MSG   "1030_TRK"   // Flight name in APRS track mode message (sent every TRACK_APRS_TX_PERIOD)
#define TRK_GPS   "1030_GPS"   // Flight name in APRS track mode GPS message (sent every TRACK_GPS_PERIOD)
#define PING_ID 30             // Value (0-255)sent for ping Identification byte (last two digits of the flight number)
...
// HIBERNATE parameters --- Launch Date: 27 May 2019, Julian Day 147, Wake Up Date: 1 Nov 2019, Julian Day 305 -----------
#define HIBERNATE_PERIOD 158       // Will Hibernate for 305-147 = 158 days
...
// Tone Parameters
#define Tone_Period 17          // Period for Ping Tone frequency (17=500 Hz)
#define Tone_PW 38              // Pulse width of Ping Tone, set dependent on Tone_Period (38 for 75msec pulse)
     ...
     if((pings_sent*TRACK_PING_TX_PERIOD) >= TRACK_APRS_TX_PERIOD * 60)      // if it is time, send an APRS message   
      ...
      else                          // Send a Ping if no APRS message was sent
       {
         ...
         switch (ping_state)        // send three tone and one encoded GPS pings for every TRACK_PING_TX_PERIOD
          {                            
            case 0:
              Output_Ping_tone();   // Send track ping tone
...
void Output_Ping_tone(void)
{
  ...
  delay(5);                             // wait 5 msec for power to stabliize

  for (int n=0; n<Tone_PW; n++)
   {      
//----------- Generate sine wave for Ping Tone using 1/4 sine wave values
//            (starts and ends with DAC voltage = 0V)
